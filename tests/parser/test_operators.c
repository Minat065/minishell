...
void test_and_operator(void)
{
    ...
    Token *op_token = create_token(TOKEN_OPERATOR, "&&");
...
void test_mixed_operators(void)
{
    ...
    // トークンストリームを作成
    TokenStream *stream = create_token_stream();
    Token *and_token = create_token(TOKEN_OPERATOR, "&&");
    and_token->value.word = strdup("&&");
    Token *or_token = create_token(TOKEN_OPERATOR, "||");
    or_token->value.word = strdup("||");
    Token *semi_token = create_token(TOKEN_OPERATOR, ";");
