...
static Token *create_token(TokenType type, const char *text)
{
    ...
    if (type == TOKEN_WORD) {
        ...
        // リダイレクトのタイプを設定
        if (strcmp(text, "<") == 0)
            token->value.redirect.redirect_type = REDIR_INPUT;
        else if (strcmp(text, ">") == 0)
            token->value.redirect.redirect_type = REDIR_OUTPUT;
        else if (strcmp(text, ">>") == 0)
            token->value.redirect.redirect_type = REDIR_APPEND;
        else if (strcmp(text, "<<") == 0)
            token->value.redirect.redirect_type = REDIR_HEREDOC;
...
void test_output_redirection(void)
{
    ...
    assert(cmd->redirects->type == REDIR_OUTPUT);
...
void test_input_redirection(void)
{
    ...
    assert(cmd->redirects != NULL);
    assert(cmd->redirects->type == REDIR_INPUT);
...
void test_append_redirection(void)
{
    ...
    t_cmd *cmd = result->ast->cmds;
    assert(cmd->redirects != NULL);
    assert(cmd->redirects->type == REDIR_APPEND);
    assert(strcmp(cmd->redirects->file, "file.txt") == 0);
...
void test_multiple_redirections(void)
{
    ...
    // 2番目のリダイレクション（出力）
    assert(cmd->redirects->next != NULL);
    assert(cmd->redirects->next->type == REDIR_OUTPUT);
...
void test_redirection_with_pipe(void)
{
    ...
    assert(cmd1->redirects->type == REDIR_INPUT);
    ...
    assert(cmd2->redirects->type == REDIR_OUTPUT);
